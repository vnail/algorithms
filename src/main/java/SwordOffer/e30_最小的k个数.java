...
public class e30_最小的k个数 {
    ...
    private static void getLeastNumbers(int[] arr, int k) {
        ...
        int index = e29_数组中出现次数超过一半的数字.partition(arr, 0, arr.length-1);
        while (index != k-1){
            if (index > k-1){
                index = e29_数组中出现次数超过一半的数字.partition(arr, 0, index-1);
            } else {
                index = e29_数组中出现次数超过一半的数字.partition(arr, index+1, arr.length-1);
            }
        }
